...
public:
    explicit ModeEngine(MovementEngine *movRef = nullptr)
        : movement(movRef), current(nullptr), currentId(MODE_EVA) {
        for (int i = 0; i < 5; i++) modes[i] = nullptr;
    }

    void setMovementEngine(MovementEngine *movRef) {
        movement = movRef;
    }
    ...
    void switchTo(EvaMode id) {
        ...
        if (movement) movement->stop(); // Force stop motors on EVERY mode switch!
